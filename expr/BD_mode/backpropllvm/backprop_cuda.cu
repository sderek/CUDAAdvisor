...
{
	cudaDeviceSetLimit(cudaLimitMallocHeapSize, 1024*1024*300); //sderek
	setup(argc, argv);
 	printf("end of main\n");  
}
...
//	cudaDeviceSetCacheConfig(cudaFuncCachePreferShared); //Du//
...
//	fprintf(stderr, " %f\n", time3-time2);
  ...
  cudaMemcpy(input_weights_one_dim, input_hidden_cuda, (in + 1) * (hid + 1) * sizeof(float), cudaMemcpyDeviceToHost);
	...
	for (int ii=0; ii<30; ii++)
		printf("%f\t", net->input_units[ii] );
	printf("\n");
	printf("\n");
	for (int ii=0; ii<30; ii++)
                printf("%f\t", input_weights_one_dim[ii] );
        printf("\n");
